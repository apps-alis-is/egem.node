    ...
    commands = {
        ...
        setup = {
            ...
            action = function(_options, _, _, _)
                ...
                if _noOptions or _options.app then
                    am.execute_extension("__egem/download-binaries.lua", { contextFailExitCode = EXIT_SETUP_ERROR })
                end
                ...
                if _noOptions or _options.configure then
                    am.app.render()
                    am.execute_extension("__egem/configure.lua", { contextFailExitCode = EXIT_APP_CONFIGURE_ERROR })
                end
                log_success("EtherGem node setup complete.")
            end
        },
        start = {
            description = "ami 'start' sub command",
            summary = "Starts the EGEM node",
            action = "__eth/start.lua",
            contextFailExitCode = EXIT_APP_START_ERROR
        },
        stop = {
            description = "ami 'stop' sub command",
            summary = "Stops the EGEM node",
            action = "__eth/stop.lua",
        ...
        validate = {
            ...
            action = function(_options, _, _, _cli)
                ...
                ami_assert(proc.EPROC, "EGEM node AMI requires extra api - eli.proc.extra", EXIT_MISSING_API)
                ami_assert(fs.EFS, "EGEM node AMI requires extra api - eli.fs.extra", EXIT_MISSING_API)
                ...
                log_success("EtherGem node configuration validated.")
            end
        },
        about = {
            ...
            action = function(_options, _, _, _)
                local _ok, _aboutFile = fs.safe_read_file("__egem/about.hjson")
        ...
        remove = {
            ...
            action = function(_options, _, _, _)
                if _options.all then
                    am.execute_extension("__eth/remove-all.lua", { contextFailExitCode = EXIT_RM_ERROR })
                    am.app.remove()
